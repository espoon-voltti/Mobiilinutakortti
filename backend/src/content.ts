...
export const RegisteredSmsContent = {
    ...
    `Hei ${recipientName}! Sinulle on luotu oma Nutakortti. Voit kirjautua palveluun kertakäyttöisen kirjautumislinkin avulla: ${link}  - Vantaan nuorisopalvelut`,
  sv: (recipientName: string, link: string) =>
    `Hej ${recipientName}! Ett Nutakort har skapats åt dig. Du kan logga in på tjänsten via denna engångsinloggningslänk: ${link}  - Vanda ungdomstjänster`,
  en: (recipientName: string, link: string) =>
    `Hi ${recipientName}! A Nuta card has been created for you. You can log in to the service via this one-time login link: ${link}  - Vantaa youth services`,
};
