...
export class SmsService {
  ...
  async sendVerificationSMS(
    ...
    const checkRegisterJuniorCalls = new Error().stack
      .split('at ')[2]
      .includes('registerJunior');
    ...
    if (!settings) {
      if (checkRegisterJuniorCalls) {
        ...
        throw new InternalServerErrorException(
          content.MessengerServiceNotAvailable,
        );
      }
    }
    ...
    const attemptMessage = await this.sendMessageToUser(
      messageRequest,
      settings.endPoint,
    );
    if (attemptMessage) {
  ...
  async sendNewSeasonSMS(
    recipients: Recipient[],
    expireDate: string,
  ): Promise<boolean> {
    const { user, username, password, batchEndPoint } =
    ...
    const attemptMessage = await this.batchSendMessagesToUsers(
      messageRequest,
      batchEndPoint,
    );
    if (attemptMessage) {
   ...
   */
  private async batchSendMessagesToUsers(
    messageRequest: TeliaBatchMessageRequest,
    endpoint: string,
  ): Promise<boolean> {
    ...
    try {
      const response = await this.httpService
        ...
        .toPromise();
      const { batchid, batchstatuscode, batchstatusdescription } =
        response.data;
      if (batchstatuscode === 1) {
        this.logger.log(
          `Batch ID ${batchid} received successfully: ${batchstatusdescription}, code ${batchstatuscode}`,
        );
        return true;
      } else {
        this.logger.log(
          `Batch ID ${batchid} failed: ${batchstatusdescription}, code ${batchstatuscode}`,
        );
      }
    } catch (error) {
      this.logger.log(
        'Batch send failed: endpoint responded with a non 200 status and error:',
        error,
      );
    }
  ...
  private async sendMessageToUser(
    messageRequest: TeliaMessageRequest,
    teliaEndPoint: string,
  ): Promise<boolean> {
    this.logger.log(`Sending SMS to ${messageRequest.to[0]}`);
    return this.httpService
      .post(teliaEndPoint, messageRequest)
      .toPromise()
      .then((response) => {
        if (response.data.accepted[0].to === messageRequest.to[0]) {
          ...
          this.logger.log(
            `Failed to send SMS to ${messageRequest.to[0]}: ${response}.`,
          );
          return false;
        }
      })
  ...
  private getExpiredMessage(
    ...
    return content.ExpiredSmsContent[lang](
      ...
      process.env.FRONTEND_BASE_URL
        ? `${process.env.FRONTEND_BASE_URL}/hae`
        : 'https://nutakortti.vantaa.fi/hae',
    );
  }
